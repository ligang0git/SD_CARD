...
typedef struct {
	...
	BYTE	drv;			/* Physical drive number *///物理驱动
	BYTE	csize;			/* Sectors per cluster (1,2,4...128) *///每个簇的扇区数目
	BYTE	n_fats;			/* Number of FAT copies (1 or 2) *///文件分配表的数目
...
#if _MAX_SS != _MIN_SS
	WORD	ssize;			/* Bytes per sector (512, 1024, 2048 or 4096) *///每个扇区的的字节数（用于扇区大于512B的flash）
#endif
#if _FS_REENTRANT
...
#endif
#if _FS_RPATH
	DWORD	cdir;			/* Current directory start cluster (0:root) *///允许相对路径时用，存储当前目录的起始簇
#endif
	DWORD	n_fatent;		/* Number of FAT entries, = number of clusters + 2 *///FAT入口簇=簇的数目+2
